...
<div class="fixed-sidebar-left">
  <ul class="nav navbar-nav side-nav nicescroll-bar">
    ...
    <li>
      <a href="javascript:void(0);" class="{{activePath('organization') ? 'active' :''}}" data-toggle="collapse" data-target="#app_org"><div class="pull-left"><i class="pe-7s-culture mr-20"></i><span class="right-nav-text">{{trans_choice('navbar.organization',2)}} </span></div><div class="pull-right"><i class="zmdi zmdi-caret-down"></i></div><div class="clearfix"></div></a>
      <ul id="app_org" class="collapse collapse-level-1">
        <li>
          <a href="{{route('organization.index')}}" class="{{activePage('organization') ? 'active-page' :''}}">
            {{__('navbar.all').' '.trans_choice('navbar.organization', 2) }}
          </a>
        </li>
        <li>
          <a href="{{route('organization.create')}}" class="{{activePage('organization/create') ? 'active-page' :''}}">
